...
class MessageAnalize:
    ...
    def messagegenerate(self):
        ...
        if self.greet():
            return '[To:{}]'.format(self.account) + self.name + "さん、本日もよろしくおねがいします。",
        if self.inword('(なん|何)だ(きみ|君)は'):
            return "[To:{}]".format(self.account) +  "なんだチミ(君)はってか!? え!? なんだチミはってか！そうです、私が変なおじさんです"
